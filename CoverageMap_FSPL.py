...
df_antpat = pd.read_excel('Coverage Map - Free Space PL Model\Antenna_Pattern.xlsx')#, encoding='utf-8_sig')
df_cellparam = pd.read_excel('Coverage Map - Free Space PL Model\Cell_Parameter.xlsx')#, encoding='utf-8_sig')
...
fs_pl_1 = 20 * np.log10(distance_hp_1) + 20 * \
    np.log10(freq_cell_1) - 27.55  # Path Loss for Cell 1
fs_pl_2 = 20 * np.log10(distance_hp_2) + 20 * \
    np.log10(freq_cell_2) - 27.55  # Path Loss for Cell 2
...
fs_pl_1 = pd.concat([fs_pl_1, fs_pl_1_NE], axis=1,
...
fs_pl_1 = pd.concat([fs_pl_1, fs_pl_1_S], axis=0,
...
fs_pl_2 = pd.concat([fs_pl_2, fs_pl_2_NE], axis=1,
                    sort=False)  # First half of the Mesh
...
# Standardize column 1st and 2nd half of the Mesh
fs_pl_2.columns = fs_pl_2_S.columns

fs_pl_2 = pd.concat([fs_pl_2, fs_pl_2_S], axis=0,
                    sort=False)  # Form a complete mesh grid
...
fs_pl_3 = pd.concat([fs_pl_3, fs_pl_3_NE], axis=1,
                    sort=False)  # First half of the Mesh
...
# Standardize column 1st and 2nd half of the Mesh
fs_pl_3.columns = fs_pl_3_S.columns

fs_pl_3 = pd.concat([fs_pl_3, fs_pl_3_S], axis=0,
                    sort=False)  # Form a complete mesh grid
...
tloss_mesh_cell_1 = fs_pl_1 + hloss_mesh_cell_1 + vloss_mesh_cell_1
tloss_mesh_cell_2 = fs_pl_2 + hloss_mesh_cell_2 + vloss_mesh_cell_2
